# 🚀 Clearity Backend - Начало работы

## 📋 Что нужно перед запуском:

1. ✅ Python 3.11+
2. ✅ Внешняя PostgreSQL база данных (имя: `clearity`)
3. ✅ OpenRouter API ключ (https://openrouter.ai/)

---

## ⚡ Быстрый старт (5 шагов)

### 1️⃣ Установите зависимости

```bash
pip install -r requirements.txt
```

### 2️⃣ Настройте .env

```bash
cp .env.example .env
```

Откройте `.env` и вставьте ваши данные:

```env
DATABASE_URL=postgresql://ваш_user:ваш_пароль@ваш_хост:5432/clearity
OPENROUTER_API_KEY=ваш_openrouter_ключ_здесь
```

**Пример:**

```env
DATABASE_URL=postgresql://postgres:mypass123@db.myserver.com:5432/clearity
OPENROUTER_API_KEY=sk-or-v1-abc123xyz
```

### 3️⃣ Инициализируйте базу данных

```bash
python db_utils.py init
```

Это создаст все таблицы и структуру в вашей БД.

### 4️⃣ Проверьте подключение

```bash
python db_utils.py check
```

Должно быть:

```
✓ Connected to PostgreSQL
✓ users
✓ sessions
... (всего 16 таблиц)
```

### 5️⃣ Запустите сервер

**Рекомендуемый способ:**

```bash
uvicorn app.main:app --reload
```

**Или на Windows:**

```bash
run.bat
```

**Или напрямую:**

```bash
python -m app.main
```

✅ **Готово!** Сервер запущен на http://localhost:55110

---

## 🧪 Проверьте работу

### Вариант 1: Браузер

Откройте http://localhost:55110/docs - увидите интерактивную документацию API

### Вариант 2: Health Check

```bash
curl http://localhost:55110/health
```

Ответ:

```json
{
  "status": "healthy",
  "database": "connected"
}
```

### Вариант 3: Автотест

```bash
python test_api.py
```

Это отправит тестовые сообщения и покажет все возможности API.

---

## 📁 Основные файлы

- **app/main.py** - точка входа приложения
- **.env** - ваши настройки (НЕ коммитить!)
- **app/schemas/db_schema.sql** - схема базы данных
- **logs/** - логи приложения (создаётся автоматически)

---

## 🆘 Проблемы?

### База данных не подключается

```bash
python db_utils.py check
```

Проверьте:

- DATABASE_URL правильный в `.env`
- База данных `clearity` существует
- Пользователь имеет права

**Подробнее:** см. `SETUP_EXTERNAL_DB.md`

### OpenRouter ошибки

- Проверьте API ключ
- Проверьте баланс на https://openrouter.ai/
- Смотрите логи в `logs/`

### Порт 55110 занят

В `app/main.py` измените:

```python
uvicorn.run(..., port=55111)  # вместо 55110
```

Или запустите с другим портом:

```bash
uvicorn app.main:app --reload --port 55111
```

---

## 📚 Дальше что?

1. ✅ Сервер работает
2. 📖 Изучите API: `API.md`
3. 🏗️ Архитектура: `README.md`
4. 🎨 Начинайте фронтенд!

---

## 🔗 Основные endpoint'ы

```bash
# Отправить сообщение
POST /api/chat
{
  "message": "I feel overwhelmed with too many projects"
}

# Получить mind map
GET /api/sessions/{session_id}/mindmap

# Получить задачи
GET /api/sessions/{session_id}/tasks
```

**Полная документация:** http://localhost:55110/docs

---

## 🛠️ Полезные команды

```bash
# Проверить БД
python db_utils.py check

# Статистика БД
python db_utils.py stats

# Сбросить БД (удалит ВСЕ данные!)
python db_utils.py reset

# Тест API
python test_api.py
```

---

## 📖 Документация

- **START_HERE.md** ← вы здесь
- **QUICKSTART.md** - подробный quick start
- **SETUP_EXTERNAL_DB.md** - настройка внешней БД
- **README.md** - архитектура и дизайн (4-layer architecture)
- **API.md** - полная документация API
- **AUTH.md** - JWT authentication + Google OAuth
- **PROJECT_STRUCTURE.md** - структура проекта

---

## ✨ Готово к разработке!

Сервер запущен, БД подключена, API работает.

Начинайте разработку фронтенда! 🎉
